...
Das MyCoRe Storage Layout ist ein eigens entwickeltes OCFL Layout für **lokale Dateisysteme**, welches ähnlich dem nativen XML Store (IFS2) arbeitet. Die Pfadberechnung von `mcrobject` und `mcrderivate` erfolgt aus der ID und einem konfigurierbaren `SlotLayout` bzw. des eingestellten `NumberPatterns` (siehe [Metadataspeicher]({{< relref "/documentation/explanation/mycore/persistence/metadata-store" >}})). Bei den Utility-Objekten (Klassifikationen, Nutzer, …) wird der Pfad aus deren Typ und dem Namen erstellt.
